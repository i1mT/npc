...
- better-sqlite3
- Server-Sent Events（单 LLM turn 短连接）
- lucide-react
...
- 模拟推进：Dashboard 使用 `src/mastra/workflows/streamed-day/` 的 day run 状态机。`/api/sim/start` 只创建或恢复 run；每个耗时 LLM 调用由 `/api/sim/turns/[turnId]/stream` 单独流式执行，避免一个全天长 SSE 连接。
- 旧模拟引擎：`src/simulation/engine.ts` 与 `src/mastra/workflows/agentic-day.ts` 保留给本地脚本和 legacy 验证路径，不再作为 Dashboard 推进一天的主入口。
...
- Tavily MCP 封面图：`src/mastra/tools/tavily-mcp.ts` 可选接入 Tavily 远程 MCP，编辑 Agent 可用 `tavily_search` 查找候选封面图；发布层仍优先使用原文已有封面图。
...
- 运行状态：`sim_day_runs` 和 `sim_llm_turns` 保存前端驱动的 streamed day run 与每次 LLM turn，`work_events` 仍是完成后业务事件的事实源。
- 日结参数：广告声誉分档 CPM、订阅日单价和收入聚合函数统一维护在 `src/simulation/formulas.ts`，工作流、结算明细和页面/API fallback 不应手写重复常量。
...
- Tavily 封面图验证：`npm run verify:tavily-cover`
...
- 当前 `src/mastra/workflows/agentic-day.ts` 已超过该上限且仍保留 legacy 长任务逻辑；Dashboard 新增逻辑应优先放入 `src/mastra/workflows/streamed-day/` 的分层模块，不要继续扩大该文件。
- 每层文件夹尽量不超过 8 个直接文件；超过时应继续按领域或职责分层。
- 任何功能、目录、核心约定或数据结构变更，都要同步更新受影响的 `AGENTS.md` 和 `docs` 文档。

## 其他

- 每次修改完代码，都需要commit，并写上对应的改动概要
