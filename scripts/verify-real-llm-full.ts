...
async function main() {
  const { dbFirst, dbRun, resetSimDb } = await import("../src/db/connection");
  ...
  const board = await getBoardMeeting(7);
  if (!board?.autoDirective) throw new Error("Day 7 board auto directive was not generated.");
  await applyBoardDirective(7, "INJECT_CAPITAL");

  await dbRun("UPDATE sim_days SET dau = 11000 WHERE day = 7");
  await runDailyWorkflow(8);
  const spawned = await dbFirst<{ agent_handle: string; joined_day: number }>(
    "SELECT agent_handle, joined_day FROM employees WHERE role_template = 'growth' AND status = 'active' ORDER BY joined_day DESC LIMIT 1",
  );
  ...
  const participated = await dbFirst<{ id: string }>(
    "SELECT id FROM work_events WHERE day = 9 AND actor_id = ? LIMIT 1",
    spawned.agent_handle,
  );
